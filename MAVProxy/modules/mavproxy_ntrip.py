...
class NtripModule(mp_module.MPModule):
    ...
    def __init__(self, mpstate):
        ...
        self.add_completion_function("(NTRIPSETTING)", self.ntrip_settings.completion)
        self.pos = None
        self.pkt_count = 0
        self.last_pkt = None
    ...
    def idle_task(self):
        ...
        if data is None:
            ...
            if (
                self.last_pkt is not None
                and now - self.last_pkt > 15
                and (self.last_restart is None or now - self.last_restart > 30)
            ):
                print("NTRIP restart")
                self.ntrip = None
            ...
            for link in links:
                for d in range(self.ntrip_settings.sendmul):
                    if random.random() * 100 < self.ntrip_settings.frag_drop_pct:
                        continue
                    link.mav.gps_rtcm_data_send(
                        flags | (fragment << 1), frag_len, send_data
                    )
            fragment += 1
            blen -= frag_len
        self.pkt_count += 1
    ...
    def ntrip_status(self):
        ...
        for id in sorted(self.id_counts.keys()):
            print(
                " %4u: %u (len %u)" % (id, self.id_counts[id], len(self.last_by_id[id]))
            )
            frame_size += len(self.last_by_id[id])
        print(
            "ntrip: %u packets, %.1f bytes/sec last %.1fs ago framesize %u"
            % (self.pkt_count, self.rate, now - self.last_pkt, frame_size)
        )
